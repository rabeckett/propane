...
%
%Models for BGP are traditionally given as per-device ranked paths where the configuration of each device is known ahead of time. For example router $r_1$ might have a first preference of $r_1 \cdot r_2 \cdot d$ and a second preference of $r_1 \cdot d$. Similarly, $r_2$ might have a first preference of $r_2 \cdot d$, and no other preference. Against this model of BGP we have the following.
%
%\begin{defn}
%Any policy expressible in BGP is expressible in \sysname.
%\end{defn}
%
%The proof of this is straightforward. Simply take the union of each router's rank 1 path, and make that the new most-prefered route in \sysname, and so on. For example, the above preferences would be encoded as:
%$$(r_1 \cdot r_2 \cdot d) \cup (r_2 \cdot d) \gg (r_1 \cdot d) $$
%Since the semantics of \sysname is such that each unique starting-ending locations should get their most-preferred path.

%\todo{I'm not sure if this is true}.
