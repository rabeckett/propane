...
\CD{ExitCust} defines paths through \CD{R1} and \CD{R2} and in the event
...
end at router \CD{A}; traffic not matching \CD{PG1}, 
but which matches \CD{PG2} must
...
locations inside the network to locations outside the network. In this case, both \CD{PG1} and \CD{PG2} will use the aggregate \CD{PG} when advertised outside the datacenter.
\todo{Say more about aggregation here.
