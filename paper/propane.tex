...
Since routing allows transit traffic (\IE, traffic can enter from
\CD{Peer} and leave through \CD{Prov}), we should define a new policy that
implements this restriction separately.  We do so by using conjunction (\AND),
...
\begin{code}
\Define PP = Peer \OR Prov
\Define PPTrans   = \Enter(PP) \OR \Exit(PP)
