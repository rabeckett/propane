...
Because the \sysname compiler has the complete user policy and topology, it is also able to efficiently check that user-specified aggregates do not black hole traffic for up to $k$ failures, where $k$ is specified by the user. The algorithm for aggregation failure safety is detailed in Section~\ref{sec:black-holes} of the appendix.
