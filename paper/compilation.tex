...
\begin{algorithm}
\caption{Failure Protection}\label{euclid}
\begin{algorithmic}[1]
\Procedure{Protect($G_1$, $N_1$, $G_2$, $N_2$)}{}
\If {$N_1.loc \neq N_2.loc$} \Return false
\EndIf
\State $q \gets Queue()$
\State $q.Push (N_1, N_2)$
\While {\textit{not} $q.Empty()$}
\State TODO
\State TODO
\EndWhile
\EndProcedure
...
TODO: how simple paths factor in 
