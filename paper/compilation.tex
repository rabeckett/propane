...
$W \cdot B \cdot C \cdot D$ path, even though this was path was not specified by the policy. This is a violation of \emph{Soundness}: The distributed implementation has used a route not specified by the policy. To make matters worse, the second preference $W \cdot B \cdot C \cdot E$ is available in the network but not being used. This is a violation of \emph{Completeness}: A path for the best possible route available exists in the network but is not being used by the distributed implementation.
We could solve the first problem by placing a filter at router $B$ to prevent using a path that goes through $D$, but we can not help with the second problem: $C$ has made the wrong choice. 
