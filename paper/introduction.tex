...
policies and the low-level configurations.  In particular,
...
device-level behaviors, such that the distributed interactions among
these devices satisfy the high-level objectives.
%
Further, policy-compliance must be ensured not only under normal network
circumstances but also in the face of failures.  The need to reason
about possible failures exacerbates an already considerable challenge
...
semantic divide between network-wide policies and device-level configuration.
They do not provide fundamentally different abstractions
from existing configuration languages and so still require operators to
manually decompose policies into device-level behaviors.

Verification tools for network configurations~\cite{batfish} have been developed as a complementary approach for reducing misconfigurations by checking that the low-level configurations match high-level operator intent. However, existing verification tools have their drawbacks: like templates, they do not help operators come up with the configuration in the first place, and moreover, they can only check correctness under concrete environment and failure scenarios, rather than under all possible environments and failure scenarios.
...
devices~\cite{ethane}. Indeed, instead of organizing networks
...
they fail to provide a coherent means to specify preferred
\emph{inter}-domain routes.  
Second, centralized control planes
require careful design and engineering to be robust to failures---one must ensure that all devices can communicate with the controller at all times, even under arbitrary combinations of failure. Even ignoring failures, it is necessary for controller systems to
scale to meet the demands of large, geographically distributed networks,
and to react quickly
to a changing environment. To address this challenge, researchers are exploring
...
departures from existing languages.  The most important difference is semantic:  the paths specified
...
intermediate language and from there to BGP configurations, including
per-device import and export filters, local preferences, MED attributes, and community tags.
We target BGP for pragmatic reasons: it is an industry
standard, and many networks use it internally as well as externally. Despite the advent of SDN, many networks will continue to
use BGP for the foreseeable future due to existing infrastructure investments, the difficulty of transitioning to SDN,
...
{\em arbitrary} failures.  In this way, we provide network operators
...
We find that our language expresses such policies easily, and that the compiler scales to topologies with hundreds of routers.
