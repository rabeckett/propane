...
Besides the languages being low-level, a fundamental reason for configuration being onerous is the semantic mismatch between the intended high-level
...
To achieve the first goal, we borrowed heavily from 
recent high-level SDN languages such as FatTire~\cite{fattire},
Merlin~\cite{foster:merlin}, and
NetKAT~\cite{netkat} by using regular expressions to specify network paths.  However, our design also contains several key
...
routes in the same notation.  
%No SDN programming language provides similar facilities.  
In addition,
we incorporate user preferences in to the algebra
of regular expressions and provide a semantics in terms of sets of ranked paths.  The preferences indicate fail-over behaviors:  among all specified paths that are still available,
the system guarantees that the highest-ranked one will be taken.  
...
intermediate language and from there to BGP configurations.  
...
However, using BGP as a vehicle for implementing high-level policies is
challenging because not all policies can be implemented correctly in
the presence of arbitrary failures.  We develop new
algorithms to detect policies that cannot be implemented
and report our findings to the operators, so they may fix the specification at compile time rather than experience undesirable behavior after configurations are deployed.
%We also analyze route aggregation to detect the possibility of failure-induced black-holes.
If the policies are implementable, our compiler automatically synthesizes per-device import and export filters, local preferences, MED attributes, and community tags.
