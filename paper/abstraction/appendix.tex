...
We use capital letters for abstract topology/product graph nodes and we use lower case letters for concrete topology nodes. By default, product graph nodes with different subscripts refer to nodes that share the same topology location (\IE, $n_1 \approx n_2$). A superscript $G^A$ refers to the abstract version (in this case of the topology), while the non-superscripted value refers to the concrete version. Given a path as a sequence of locations $p = l_1, \dots, l_k$ we often apply the graph homomorphism $f$ to the path by writing: $f(p) = f(l_1), \dots, f(l_k)$.

\section{Proof Overview}

The goal of the proof is to show that the concretization and compilation functions commute:
%
$$\Con(\Compile(pol,G^A),\Gamma,f,G) = \Compile(\Con(pol,\Gamma,f), G)$$ 
%
The proof proceeds in the following fashion. First, we establish several facts about the relationship of the concrete and abstract product graphs given the definition for building the product graph. Assumming ...
%
$$t = \$x \implies pfx \in \Gamma(x)$$
$$\CompilePg(t \Path r_1 \Prefer \ldots \Prefer r_n, G) = (t, PG, \Pref)$$
$$\CompilePg(pfx \Path r_1 \Prefer \ldots \Prefer r_n, G^A) = (pfx, PG^A, \Pref^A)$$
%
Then several relationships hold between $PG$ and $PG^A$ as well as between $\Pref$ and $\Pref^A$. Using these facts, we then establish that concretization and compilation commute for an individual constraint p:
%
$$\Con(\Compile(p, G^A), \Gamma, f, G) = \Compile(\Con(p,\Gamma,f), G)$$
%
This proof proceeds by cases on the constraint p. Finally, this theorem is lifted to policies $pol = p_1, \ldots, p_n$ using the previous proof to obtain the final theorem.
...
%
%$$t = \$x \implies pfx \in \Gamma(x)$$
%$$\CompilePred((t,PG,\Pref), G) = t \rightarrow xs $$
%$$\CompilePred((pfx,PG^A,\Pref^A), G^A) = pfx \rightarrow ys$$
%$$xs = ys$$
...
$$\Con(\Compile(pol,G^A),\Gamma,f,G) = \Compile(\Con(pol,\Gamma,f), G)$$ 
That is -- we can perform compilation directly over the abstraction and only substitute for the concrete values on after compilation is complete. Note that the theorem states that the resulting mBGP policies are actually syntactically equivalent and not semantically equivalent. First we need a few helper theorems about compilation.
