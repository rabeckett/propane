...
\begin{figure*}[t!]
  ...
  \begin{tikzpicture}
    \inference{0}{L$(j,k)$}{$e_1 > 0$}{S$(min(j*k,e_1),1)$}{I-out1}
    \inference{4.5}{A$(j,k)$}{$e_2 > 0$}{A$(1,min(j,e2))$}{I-out2}
    \inference{9}{L$(j,k)$}{$e_1=n$}{A$(min(j*k,n),1)$}{I-mesh1}
    \inference{13.5}{L$(j,k)$}{$e_1=n$}{A$(1,j)$}{I-mesh2}
  ...
  \begin{minipage}[t]{.5\linewidth}%
  \begin{tikzpicture}
    \inference{0}{L$(j,k)$}{$e_1 > 0 \wedge e_2 > 0$}{S$(min(j*k,n-\dfrac{(m-j)*e_1}{e_2}),1)$}{I-out3}
    \inference{10}{L$(j,k)$ ~~~ L$(j- m ~\text{mod}~ \big(\dfrac{n}{e_1}\big),1)$}{$e_1 > 0 \wedge e_2 > 0$}{}{I-striping}
  \end{tikzpicture}
  \end{minipage}
