...
\noindent
%
Suppose now we additionally add the valleyfree constraint that traffic never has takes a path with a down-up in it. Figure~\ref{fig:example-inference} shows the abstract product graph representation for this routing policy. The inference algorithm starts from the node $(TG,0,0)$ with the initial fact $S_P S_{TG}(1,\infty)$ (\IE, no restriction on the number of disjoint paths initially). 
%
...
\ryan{inference should only infer some}
