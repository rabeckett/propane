...
\noindent
%
Each label $L \in \{S,A\}$ is either $S$, which stands for ``some" or is $A$, which stands for ``all". There is one label in the nesting for each pod scope in the network abstraction hierarchy. So for a given node, $L_{X_1}$ would correspond to the outermost pod scope, while $L_{X_n}$ would correspond the the node's label. The value $j$ tells us the number of concrete nodes in the group that are reachable, and the value $k$ tells us the number of disjoint paths to each of the $j$ nodes such that all $j*k$ paths are disjoint.
