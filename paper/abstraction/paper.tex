...
At a high level, the translation from the PG representation to per-device templates that run the distributed BGP protocol involves the following steps: (1) Every BGP message is tagged to record the state of the product graph as messages are passed between routers. The tags allow BGP to only search for valid paths by dropping messages that do not correspond to any edge in the PG. (2) To ensure that BGP always converges to the \emph{most preferred} paths in the network, we must rank route advertisements for each router by giving them a preference such that, under those preferences, the network as a whole satisfies the \sysname policy's end-to-end network preferences. 

We introduce a simple, vendor-independent BGP configuration language called \mbgp and describe a compilation function that uses the inferred preference ordering to generate concrete configurations from a concrete topology, or templates from an abstract topology. Templates can then be soundly instantiated for a concrete network by replacing instances of abstract neighbors with the union of all concrete neighbors under the inverse homomorphism. We prove that concretization and compilation commute. By modifying compilation slightly we can further ensure that every configuration depends only on its immediate neighbors. This guarantees that any change made to the concrete topology will result in a minimal number of changes to the configurations.
...
Community tags serve as a simple form of \emph{history}, and operators routinely use such tags to implement policy. For example, operators might record where an advertisment enters the network using community tags and then conditionally block the export of the advertisment later to other peers to prevent becoming a transit point between peers.

The idea for template generation is to use community tags to record the state of the automata from the Product Graph in every BGP announcement and update these tags whenever messages are passed between routers. Such a tagging mechanism is used to ensure that BGP only considers paths that are allowed by the \sysname policy (\IE, paths in the PG). Routers will allow advertisments that correspond to an edge in the PG and block any advertisments that do not.

For example, in the concrete PG in Figure~\ref{fig:example-compilation}, router S1 appears in two different PG nodes: $(S1,1,0)$ and $(S1,0,1)$. These nodes have two different peers that can send them messages in the PG: $(N1,0,0)$ and $(N2,0,0)$. Therefore, S1 will allow an advertisement from peer $N1$ and will also allow an advertisement from peer $N2$. If S1 uses the path advertised by neighbor N1, then it will add the tag $(1,0)$ to the advertisement before sending this to its neighbors A1 and A2. If S1 uses a path advertised from neighbor N2, then it will add the tag $(0,1)$ instead. Similarly, router A1 appears in two separate PG nodes. It will not block advertisements from S1 if they either have the $(1,0)$ tag or the $(0,1)$ tag attached. In either case, it will not modify the tag before exporting the route to its neighbors.

%If A1 chooses a path with tag $(1,0)$ attached, then it will export the path to its neighbors without modifying the tag. If A1 chooses a path with tag $(0,1)$ attached, it will also export the path to its neighbors without modifying the tag.
...
\begin{defn}
We write $m \leq n$ if the subgraph reachable from $m$ and $n$ respectively form a simulation relation with respect to $\geq_{rank}$.
\end{defn}

More specifically, we say that $m \leq n$ if $m \geq_{rank} n$ and for every transition $n\overset{l}{\rightarrow}n'$ from PG node $n$ there exists a transition $m\overset{l}{\rightarrow}m'$ from $m$ and $m' \leq n'$.
%
Intuitively, the $m \leq n$ relation means advertisements received for node $m$ can safely be preferred over those received for node $n$ after accounting for the network-wide impact of the choice. For example in Figure~\ref{fig:example-compilation}, S1 can receive advertisements in two different contexts ($(S1,1,0)$ and $(S1,0,1)$) and must choose between messages received in these different contexts. Advertisements are preferred in state $(S1,1,0)$ because it results in a better path for S1 ($(S1,0,1) \geq_{rank} (S1,1,0)$) and downstream routers such as A1 will also obtain paths no worse than if S1 had choosen $(S1,1,0)$ instead. The policy is guaranteed to be safe from failures because if a link fails in the topology, then $m \leq n$ will still hold since any transition that becomes unusable for $m$ also becomes unusable for $n$. That is $m \leq n$ before the failure implies $m \leq n$ after the failure.

For each router, its corresponding PG nodes are then sorted according the the $\leq$ operator. If the operator defines a total order on PG nodes, then the compiler can simply prefer advertisements from peers of node $m$ over those of $n$ whenever $n \leq m$. However, if $\leq$ does not form a total order, then the policy is rejected as being potentially unsafe under some failure conditions.
