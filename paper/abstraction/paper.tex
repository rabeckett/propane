...
Configuring a BGP network requires configuring, among other things, the import and export policies for each router such that traffic flows in a way that is compliant with the network's global policies. We demonstrate the difficulty of this task using an example.
...
Consider the data center network in Figure~\ref{fig:example}. The boxes in the diagram
represent \emph{switches}. Using terminology for fat tree networks~\cite{fattree}, T[1--8] denote top-of-rack (ToR) switches, A[1--8] denote aggregation switches, and S[1--2] denote spine switches. The spine switches are connected to the
rest of the Internet; any traffic to or from the outside world flows through them.  The aggregation switches 
bridge the spines to the ToRs which attach to a set of servers (``a rack''). P[1--8] denote address prefixes for services hosted in each rack.
