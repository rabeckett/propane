...
Our topology abstractions can also capture concrete topologies; there will be a one-to-one correspondence between abstract and concrete nodes/edges. This allows operators to define complex networks in which some (e.g., legacy) parts of the network can not evolve while others can.
...
\begin{figure*}[t!]
  ...
  \begin{minipage}[t]{.3\linewidth}
    ...
    \vspace{1em}
    \includegraphics[width=.62\columnwidth]{figures/topology-con}
  \end{minipage}%
  %
  \begin{minipage}[t]{.3\linewidth}
    \hdr{Abstract Topology}{}
    \vspace{1em}
    \includegraphics[width=.505\columnwidth]{figures/topology-abs}
  \end{minipage}%
...
At a higher level, what is happening is that each inference rule is attempting to learn the maximum possible fault tolerance information possible as a function of the symbolic inputs, and $\nu$Z~\cite{z3opt} will minimize this maximum by accounting for all possible topologies that meet the abstraction. Facts learned with $j=0$ or $k=0$ are discarded.
...
For example, in the concrete PG in Figure~\ref{fig:example-compilation}, router S1 appears in two PG nodes: $(S1,1,0)$ and $(S1,0,1)$. These nodes have two peers that can send them messages in the PG: $(N1,0,0)$ and $(N2,0,0)$. Therefore, S1 will allow advertisements from both peer $N1$ and peer $N2$. If S1 uses the path advertised by neighbor N1, then it will add the tag $(1,0)$ to the advertisement before sending this to its neighbors A1 and A2. If S1 uses a path advertised from neighbor N2, then it will add the tag $(0,1)$ instead. Similarly, router A1 appears in two separate PG nodes. It will allow advertisements from S1 if they either have the $(1,0)$ tag or the $(0,1)$ tag attached. In either case, it will not modify the tag before exporting the route to its neighbors.
...
\begin{defn}
Let $m \geq_{rank} n$ be a relation over PG vertices that holds iff $\min (\Pref(m)) \geq \min (\Pref(n))$ or $\Pref(n) = \emptyset$. Intuitively $m \geq_{rank} n$ means that paths ending at node $m$ have higher automata rank and are thus no better than paths ending at $n$.
%
\end{defn}
\noindent
%
...
Although the disjoint path analysis takes place over the PG, each application of the inference rules from Figure~\ref{fig:inference-rules} depends on the topology locations, but not the automata states. Therefore, we lazily apply the rules and cache the satisfiability and minimization calls to $\nu$Z after their first use. Furthermore, the cached results are shared across different prefixes, each of which may have a unique PG representation.
