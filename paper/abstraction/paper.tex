...
Link failures in networks occur frequently; it is not uncommon for a large network to experience dozens of such failures in any given day~\cite{dc-failure-study}. However, existing tools~\cite{condor,propane} can reason about fault-tolerance only for concrete topologies. The problem for \sysname is harder because it must verify that all possible concrete instantiations of an abstract topology satisfy the fault-tolerance requirements, which requires a fundamentally different approach.
...
Finally, rules \infrule{I-local} and \infrule{I-global} reason across pod hierarchies. \infrule{I-local} says that if there is an inference from $L_m(j,k)$ to $L_n(j',k')$ on the left, then if both nodes are in the same pod P, we can leave all labels up to P unchanged. \infrule{I-global} says that when the edge goes across pods, we can infer the fact for all pods Q since the multiplicities apply uniformly for each pod.
...
The abstract disjoint path analysis starts from a fixed source location $src$ and repeatedly tries to apply every inference rule from Figure~\ref{fig:inference-rules} until it reaches a fixed point. The algorithm applies an inference rule when the rule condition is valid given the abstract topology constraints. This ensures that the condition holds for all concrete instantiations of the topology.  Because the inference rules may continue to yield larger and larger symbolic expressions, we make the following observation to ensure termination: for any fact learned of the form $L(j,k)$, it is sound to instead infer $L(j',k')$ if $j' \leq j$ and $k' \leq k$. Therefore, for each inference $L(j,k)$ we minimize the symbolic expressions for $j$ and $k$ subject to the topology constraints using the optimizing SMT solver $\nu$Z~\cite{z3opt}. Invariants learned with $j=0$ or $k=0$ provide no useful information and are discarded.
...
The algorithm was able to infer that any spine router has at least 1 disjoint path and any TL router has at least 2 disjoint paths to a TG router. In this case, the analysis is precise. There exists a concrete network, namely the datacenter from Figure~\ref{fig:example2}, where a single failure can disconnect a global ToR from a spine router due to the valley-free constraint. The bound on the fault tolerance between ToR and spine routers can also be used to determine that an aggregation-induced black hole may arise at the datacenter boundary after a single failure in some concrete instantiation.
...
For both recursive topologies, the analysis can accurately determine reachability (i.e., at least 1 disjoint path) but is conservative in determining k-disjoint paths.
