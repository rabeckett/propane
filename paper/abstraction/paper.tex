...
Each such role is codified as a \emph{configuration template}---essentially
a macro that may be instantiated with different concrete values to
generate a configuration for each device that plays the role.

Such templates have no particular semantics other than the semantics
that one obtains from instantiating a template's parameters.  Moreover,
template construction is a manual process:  network engineers do their
best to design templates by hand that will work effectively in the context 
of their particular network topology.  However, there is no support for
validating key properties such as connectivity and fault tolerance, 
...
Unfortunately, each change to a router configuration typically incurs
...
To combat these problems, in this paper, we present \sysname, a 
new high-level language designed to 
provide improved support for the configuration of large, structured networks.
\sysname allows network operators to define abstract network topologies that
classify devices in terms of abstract roles and specify the connectivity
invariants satisfied by each role.  For instance, a programmer may
...
In order to specify \emph{routing policy} for such abstract networks,
\sysname borrows notation from the recently-defined \propane
programming language~\cite{propane}.  However, whereas \propane policies
refer to individual devices, \sysname policies allows users to
refer to abstract roles.  The \sysname compiler compiles these role-based
...
To summarize, the central contribution of this paper are:

\begin{enumerate}
\item Design of new network topology abstraction mechanisms involving roles and
connectivity invariants.
\item Design of two new algorithms for analyzing abstract networks: one which
...
evolution with minimal change to existing devices.
...
\sysname builds on the Propane network configuration synthesis tool, by supporting both synthesis for and reasoning over parameterized classes of topologies and their interactions with user-specified routing policy. 
...
\ryan{TODO: this was copied from previous paper for reference}

The Product Graph is constructed by adding an edge from state $m = (l_m, a_1, \dots, a_k)$ to $n = (l_n, b_1, \dots, b_k)$ whenever $\sigma_i(a_i, l_n) = b_i$ for each $i$ and $(l_m,l_n) \in E$ is a valid topology link.
%
...
%
We write $(m \leq_{rank} n)$ if either $P(m) = P(n) = \emptyset$ or $min ~ P(m) \leq min ~ P(n)$, which means that paths ending at PGIR node $m$ are better (lower rank) than paths ending at $n$.
%
For a Product Graph if vertex $m,n = (l, \dots) \in V'$, we say that $m$ and $n$ are shadows, written: $m \approx n$.
...
\subsection{Product graph distributed configurations}

The translation from the Product Graph to per-device configurations that run the distributed BGP protocol consists of 2 steps. The first step is to generate per-device BGP import and export filters based on the edges in the Product Graph. The idea is to attach a tag to every route advertisement that routers send to their neighbors that encodes the current state of the automata. For example... \ryan{show example here}. This ensures that BGP will only find paths that exist in the Product Graph, and are thus allowed by the policy.

The second step is to find, for each router, a total ordering of Product Graph neighbors that tells it which advertisements to prefer in the routing protocol to others. In general, because BGP is distributed, each router does not have a full picture of the entire network when choosing which path to use. It is therefore possible for a router to choose a path that results in a less preferred network-wide path even though a better one is available in the network. In general, finding a collection of preferences to ensure correct end-to-end paths for all failures is a hard problem. The Propane tool introduced a conservative, yet effective algorithm for determining route preferences based on the following simulation relation:
