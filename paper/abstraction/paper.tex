...
\authorinfo{Ryan Beckett}{Princeton}{}
\authorinfo{Ratul Mahajan}{Microsoft}{}
\authorinfo{Todd Millstein}{UCLA}{}
\authorinfo{Jitendra Padhye}{Microsoft}{} 
\authorinfo{David Walker}{Princeton}{} 
...
faster than systems that operate over concrete topologies.
...
topologies. Problems that occur may cause operators to make small, non-uniform, one-off changes to particular routers,
which defeats the purpose of a template system.  An even worse situation occurs when
operators feel obligated to update many (non-local) devices to extend or change their network.  
...
within data centers due to its scalability properties.  However, it is far from the only control
plane protocol; synthesis of configurations for other common protocols such as OSPF is beyond the 
scope of this paper.

During synthesis, our compiler performs an important analysis over abstract topologies to determine
the fault tolerance properties of the specified policy.  This analysis determines the
number of failures require to disconnect one abstract location from another given the constraints
specified by the abstract topology.  The analysis is specified by a collection of inference rules
and we prove these inference rules are sound: any concrete instance of the abstract topology will
adhere to the given fault tolerance property. \dpw{Ryan: check this.}
...
provably-correct BGP templates and is much faster
...
\noindent%
%
The Local policy adds the $\CD{\Always(\In)}$ constraint for each local prefix described by the template variable $\CD{\$LP}$. This constraint ensures that traffic follows a path that matches an internal location (\IE, in the data center) at each hop of the path.
%
...
%
The PG is a tuple ($G'$, $start$, \Pref) where $G' = (V',E')$ has
vertices $V' \colon V \times Q_1 \times \dots \times Q_j$,
edges $E' \colon V' \times V'$,
a unique starting vertex $start$,
and a preference function $\Pref \colon V' \rightarrow 2^{\set{1, \dots, j}}$, mapping nodes in the PG to a set of path ranks.
...
%
The preference function $\Pref(m)$ denotes the rank of paths through the PG ending at node $m$ and is defined as $\Pref(m) = \set{i~\vert~q_{m_i} \in F_i}$.
%
Finally, we write $\Topo{m} = l$ to extract the topology location from a PG node, when $m = (l, q_{m_1}, \dots, q_{m_k}) \in V'$.
...
%
We frame satisfying the fault-tolerance policy provided by the user as an analysis problem over the structure of the PG. In particular, we develop an analysis that uses information embedded in the abstract topology to infer bounds on the number of edge-disjoint paths between pairs of concrete nodes.

%
For each node in the abstract PG,
the idea is to infer facts learned about the number of disjoint paths
...
\begin{figure*}[t!]\small%
  ...
  \hdr{Compilation to ABGP}{}
  \vspace*{-1\baselineskip}
  %
  \[ \begin{array}{l}
     \mathrm{compile}_\mathrm{ABGP}( (t_1,PG_1,ord_1), \dots, (t_k,PG_k,ord_k), G ) = \\
     ...
     ~~~~~~~~~~~~~~~~~~~~~ ma = \mathrm{ord}_i(m) : (in,q_n) \rightarrow (out,q_m) ~] ~] ~] \\
     \\
     \mathrm{compile}( p_1, \dots, p_k, G) = \\
     ~~~~~~ \mathrm{compile}_\mathrm{ABGP}(\mathrm{compile}_\mathrm{PG}(p_1,G), \dots, \mathrm{compile}_\mathrm{PG}(p_k,G), G) \\
  \end{array} \]%
  \end{minipage}%
  ...
  \caption{Abstract BGP syntax (left), and compilation from product graphs (right).}
...
At a high level, the translation from the PG to per-device templates that run the distributed BGP protocol involves the following high-level ideas: (1) Every BGP message is tagged to record the state of the product graph as messages are passed between routers. The tags allow BGP to only search for valid paths in the PG by dropping messages that do not correspond to any edge in the PG. (2) To ensure BGP always finds the \emph{most preferred} paths in the network, we must rank messages for each router in the network by giving them a preference such that, under those preferences, the network as a whole satisfies the \sysname policy's end-to-end network preferences. (3) We introduce a simple, vendor-independent BGP configuration language called mBGP and describe a compilation function that uses the inferred preference ordering to generate concrete configurations from a concrete topology, or templates from an abstract topology. (4) Templates can then be soundly instantiated for a concrete network by replacing instances of abstract neighbors with the union of all concrete neighbors under the homomorphism. We prove that concretization and compilation commute. (5) By modifying compilation slightly we can further ensure that every configuration depends only on its immediate neighbors. This guarantees that any change made to the concrete topology will result in a minimal number of changes to the configurations.

We now look at each of these steps in turn.
 
\para{Tagging and Filtering}

The BGP routing protocol standard allows for the use of \emph{community tags}, collections of 32-bit integer tags that can be attached or removed arbitrarily from routing advertisements. 
Community tags serve as a simple form of \emph{memory}, and operators routinely use such tags to implement policy (e.g., to prevent transit traffic by recording where a message enters the network and blocking upon egress accordingly).

The idea for template generation is to use community tags to record the state of the automata from the Product Graph in every BGP announcement and update these tags whenever messages are passed between routers. Such a tagging mechanism is used to ensure that BGP only considers paths that are allowed by the \sysname policy, routers will allow advertisments that correspond to an edge in the PG and block any advertisments that do not.

For example, in the concrete PG in Figure~\ref{fig:example-compilation}, router S1 appears in two different PG nodes: $(S1,1,0)$ and $(S1,0,1)$. These nodes have two different peers than can send them messages in the PG: $(N1,0,0)$ and $(N2,0,0)$. Therefore, S1 will not block an advertisement from peer $N1$ and will also not block an advertisement from peer $N2$. If S1 uses the path advertised by neighbor N1, then it will add the tag $(1,0)$ to the advertisement before sending this to its neighbors A1 and A2. If S1 uses a path advertised from neighbor N2, then it will add the tag $(0,1)$ instead. Similarly, router A1 appears in two separate PG nodes. It will not block advertisements from S1 if they either have the $(1,0)$ tag or the $(0,1)$ tag attached. If A1 chooses a path with tag $(1,0)$ attached, then it will export the path to its neighbors without modifying the tag. If A1 chooses a path with tag $(0,1)$ attached, it will also export the path to its neighbors without modifying the tag.


\para{Preference Search}

Tagging and filtering according to the PG with community values in BGP forces the protocol to only search for paths to the destination that are allowed by the policy (\IE are paths in the PG). However, finding \emph{some} path to the destination is not enough. We must ensure that each router uses its \emph{best} available path to the destination according to the policy. Further, this must continue to hold as additional elements of the network fail. In the example from Figure~\ref{fig:example-compilation}, the policy was $\True ~\Path~ \CD{\Exit(N1\Prefer N2)}$, which indicates that paths through N1 should be used whenever possible, and paths through N2 should only be used as a backup. If router S1 allows advertisements from both N1 and N2 but does not prefer one advertisement over another, it might end up choosing to use a path through N2 even though a more preferred path through N1 exists and is available\footnote{ties between equally preferred paths can be broken nondeterministically}. On the other hand, if the S1-N1 link fails, then the best available path is through N2.

To enforce correct network-wide preferences, we use the BGP local-preference attribute, which allows routers to prefer routes from one neighbor (or with some tag) over routers learned from another neighbor. The challenge is to find a collection of device-local preferences that correctly enforce the network-wide preferences from the policy for all failures.

The idea then is to search for such a device-local preference function for each router that totally order advertisements from different neighbors (possibly with different tags). For example, to satisfy the policy that leaving through N1 is preferred to leaving through N2, S1 should prefer a message it hears from N1 over N2. Similarly, A1 should prefer routes tagged with $(1,0)$ over those with $(0,1)$. Intuitively, S1 should prefer a message from N1 because it results in an accepting state for automata 1, which is more preferred than automata 2. Further, it can result in downstream routers such as A1 receiving a message in a more preferred state $(A1,1,0)$ instead of $(A1,0,1)$.

In general, because BGP is distributed, each router does not have a view of the entire network when choosing which path to use and finding a collection of preferences to ensure correct end-to-end behavior for all failures is a hard problem. \Propane introduced a conservative search strategy for determining route preferences that works well in practice. To make it work for abstract topologies, we modify the search based on the following observation about the PG structure:

\begin{defn}
Let $m \geq_{rank} n$ be a relation over PG vertices that holds iff $\min (\Pref(m)) \geq \min (\Pref(n))$ or $\Pref(n) = \emptyset$. Intuitively $m \geq_{rank} n$ means that paths ending at node $m$ are no better than paths ending at $n$.
%
\end{defn}
\noindent
%
...
\begin{defn}
We write $m \leq m'$ if the subgraph reachable from $m$ and $m'$ respectively form a simulation relation with respect to $\geq_{rank}$.
\end{defn}

The existence of multiple nodes in the PG for the same topology location (\EG, $(S1,1,0)$ and $(S1,0,1)$) indicates that S1 can receive advertisements in different contexts and must choose between messages received in these different contexts. Node $m$ is preferred to $m'$ if, $m' \geq_{rank} m$ and, transitively, for each neighbor of $m'$, $m$ has a corresponding neighbor that is more preferred.
Algorithm~\ref{alg:failures} shows a simple procedure for determining if a PG node $N1$ can safely be preferred to a PG node $N2$.

For each router, its corresponding PG nodes are then sorted according the the $\leq$ operator. If $\leq$ defines a total order on PG nodes, then the compiler can simply prefer advertisements from peers of node $m$ over those of $m'$ whenever $m' \leq m$. This condition ensures that BGP devices will converge to the most preferred paths in the network even when failures occur. However, if $\leq$ does not form a total order, then the policy is rejected as being potentially unsafe under some failure conditions.
