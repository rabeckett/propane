...
\newcommand{\sysname}{{\small \sf Diesel}\xspace}
...
\title{Abstraction Compilation Techniques}
...
%
% (4) lets us scale compilation/algorithms to large-scale topologies


\textbf{Abstract---}
We develop \sysname, a tool for automated reasoning about end-to-end properties of abstract network topologies. \sysname allows network operators to write a centralized routing policy over a high-level abstraction of their network topology. From such a specification, it generates a distributed collection of BGP router configurations that are guaranteed to implement the centralized routing policy for any concrete instantiation of the abstract topology, and for any combination of network failures in the resulting concrete topology.
%
...
& Network configuration is difficult/error-prone. Operators think of policy at 
  a high level of abstraction, yet implementations:
...
& To this end, we develop a language called \sysname to simplify the operators task.
&& \sysname allows the operator to define high-level network abstractions to reflect
   ...
   abstraction.
&& We develop an intermediate graph-based representation for compiled, abstract policies,
   and show how to compile \sysname policies to this representation. 
&& We develop several algorithms that operate over this graph representation and verify 
   if properties, such as reachability or no black holes under k-failures, are guaranteed
   to hold under any possible concrete instantiation of the network.
...
Placeholder for formal language definition
...
& Give formal semantics of abstract, labelled graph
&& i.e., if a concrete graph + homomorphism is valid
   given an abstract, labelled graph
...
&& Next, show the Product Graphfor both the concrete and abstract policies.
...
\begin{easylist}[itemize]
& Reachability analysis (i.e., are all locations specified in the policy reachable
  after taking topology into account) 
&& Inference rules for reachability
&& Fixed point computation using SMT solver

& Reachability under k-failures (e.g., aggregation black holes)
&& translation to ILP problem
\end{easylist}
