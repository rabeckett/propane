...
Second, \sysname can scale substantially better than its nearest competitor Propane, which operates over concrete topologies. As the number of devices in a
...
%      & & Pairs & Pairs & Pairs & Pairs \\ \hline
      ...
      Fattree~\cite{fattree} & -- & \cmark & \cmark & \cmark & \Conserv  \\ \hline
      Facebook~\cite{facebook-fattree} & -- & \cmark & \cmark & \cmark & \cmark \\ \hline
...
We use a fat tree topology and scale the size of the topology according the number of pods $k$. The abstract topology uses one abstract node for each tier of the data center with an additional node for separate local and global ToRs.
...
This goal aligns better with operators' mental models and tools and permits network evolution with minimal disruption to existing routers. In the process, we develop new abstractions for network topologies, compilation algorithms that generate templates, and analysis techniques that operate over classes of networks. 
...
A complementary approach to reducing configuration errors is by analyzing the forwarding rules or the configuration of a network to ensure the absence of (a class of) bugs~\cite{anteater,hsa,veriflow,feamster+:rcc,ipassure,batfish,bagpipe,arc,era,surgery}. Our focus, in contrast, is on a correct-by-construction approach. It has the downside that operators must learn to express policies in a new language, but it saves them from the challenging and time-consuming task of manual configuration generation.
