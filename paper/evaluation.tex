...
Figure~\ref{fig:config-min} shows the size of the configuration generated by \sysname, again as a function of network size. The naive translation of PGIR to ABGP generates large ABGP policies. To offset this, the compiler performs configuration minimization both during and after configuration generation.
...
The figure shows that minimization is highly effective for both the data center and backbone policies. In all cases, minimized policies are a small fraction of the size of their non-minimized counterparts. 
