...
%DIF DEL paper.tex          Fri Jun 10 11:03:48 2016
%DIF ADD paper-edited.tex   Tue Jun 14 18:22:47 2016
%DIF < \documentclass{sig-alternate-10pt}
%DIF -------
%\documentclass{sig-alternate-10pt} %DIF > 
\documentclass[10pt]{sigalternate052015} %DIF > 
%DIF -------
...
\DIFaddbegin \setcopyright{acmcopyright}
...
\DIFaddend \special{papersize=8.5in,11in}
...
%DIF < \category{CR-number}{subcategory}{third-level}
%DIF < % general terms are not compulsory anymore,
%DIF < % you may leave them out
%DIF < \terms
%DIF < term1, term2
%DIF < \keywords
%DIF < keyword1, keyword2
...
%DIF > % general terms are not compulsory anymore,
%DIF > % you may leave them out
%DIF > \terms Network Programming Language, Distributed Program Synthesis
%DIF > \keywords Propane Language, BGP, Synthesis, Compilation

%DIF > =====================================================
%DIF > 
%DIF > 
\DIFaddend %  **Introduction**
%
%
%=====================================================
...
\CD{R2}-\CD{Prov} link but to announce routes with communities for the \CD{R2}-\CD{Cust} and \CD{R1}-\CD{Cust} links. \DIFaddbegin \DIFadd{A similar policy must be configured for }\CD{R2}\DIFadd{. }\DIFaddend Finally, to implement P4 and P5, the operators will have to compute and configure appropriate prefix- and AS-path-based \DIFaddbegin \DIFadd{import and export }\DIFaddend filters at each router.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}[t!]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\columnwidth]{figures/example2}
%DIFDELCMD <   \caption{Policy-compliance under failures is difficult.}
%DIFDELCMD <   \label{fig:example2}
%DIFDELCMD <   \vspace{-1em}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
}\CD{PG1} \DIFadd{\Path \text{ }}\End\DIFadd{(}\CD{A}\DIFadd{), where }\CD{PG1} \DIFadd{falls under the }\CD{PG} \DIFadd{aggregate. Figure~\ref{fig:aggregation-safety} shows the simplified PGIR. Since the compiler knows aggregation will occur at $X$, and it knows that the }\CD{PG1} \DIFadd{prefix will originate at $A$, we can compute the number of failures it would take to disconnect $A$ from $X$. We could remove the $A$--$D$--$X$ path first. We would then need to remove any other $A$--$D$ or $D$--$X$ links from the PGIR (in this case none). Next, we could remove the links along the $A$--$C$--$X$ path, repeating the process. Because $A$ is then disconnected from $X$, the compiler knowns that 2 is a lower bound on the number of failures for aggregation safety for prefix }\CD{PG1}\DIFadd{. This process is repeated for other aggregation locations (e.g., $Y$).
}
