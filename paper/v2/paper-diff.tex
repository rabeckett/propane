...
%DIF ADD paper-edited.tex   Tue Jun  7 16:14:21 2016
...
leave through }\CD{Peer} \DIFadd{and so then }\CD{Prov}\DIFaddend .  As we
...
}\DIFdelend and all other prefixes (\CD{true}).\footnote{Policy statements are processed in
...
\DIFdelbegin \DIFdel{statements }\DIFdelend \DIFaddbegin \DIFadd{the statement }\DIFaddend involving \CD{PCust}\DIFdelbegin \DIFdel{and }%DIFDELCMD < \CD{PInt}%%%
\DIFdelend , it is interpreted as
\DIFdelbegin %DIFDELCMD < \CD{\True{} \AND{} \NOT{}PCust \AND{} \NOT{}PInt}%%%
\DIFdelend \DIFaddbegin \CD{\True{} \AND{} \NOT{}PCust}\DIFaddend .}
...
Line 4 }\DIFdelend applies to any other traffic, and allows \DIFdelbegin \DIFdel{any routes that leave through a peer with a preference for customers over peers over providers}\DIFdelend \DIFaddbegin \DIFadd{the traffic to leave through any direct neighbor with the usual preferences of }\CD{R1} \DIFadd{over }\CD{R2} \DIFadd{over }\CD{Peer} \DIFadd{over }\CD{Prov}\DIFaddend . To summarize our progress, the \CD{Routing} policy
...
\end{code}
\noindent
\DIFadd{Here we define transit traffic between $X$ and $Y$ as traffic that enters the network through $X$ or $Y$ and then leaves the network through $X$ or $Y$. Similarly, we define customer transit for customer $X$ and provider $Y$ as traffic that later goes through both $X$ and $Y$ after leaving our network. Using these two new abstractions, we can now implement policies P2 and P5 with the following constraint.
}\DIFaddend 
...
\end{code}
\DIFaddend \noindent
The \CD{NoTrans} constraint above \DIFdelbegin \DIFdel{says that for any prefix, traffic should never both enter and exit the network from }\DIFdelend \DIFaddbegin \DIFadd{requires that all traffic not follow a path that transits the network between }\DIFaddend \CD{Peer} \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend \CD{Prov}. \DIFdelbegin \DIFdel{Similarly }\DIFdelend \DIFaddbegin \DIFadd{Additionally, }\DIFaddend it prevents traffic from ever following paths that leave our network and later \DIFdelbegin \DIFdel{passing }\DIFdelend \DIFaddbegin \DIFadd{pass }\DIFaddend through both \CD{Prov} and \CD{Cust}.  To implement both \CD{Routing}
...
\DIFaddbegin \DIFadd{The special keyword }\Out{} \DIFadd{represents any location outside the user's network, and the keyword }\In{} \DIFadd{represents any location inside the network.
...
As before, once \sysname compiles the policy, it is guaranteed to \DIFdelbegin \DIFdel{hold }\DIFdelend \DIFaddbegin \DIFadd{be policy-compliant }\DIFaddend under all possible failure scenarios. \DIFaddbegin \DIFadd{In addition, }\DIFaddend \sysname \DIFdelbegin \DIFdel{also ensures safety against }\DIFdelend \DIFaddbegin \DIFadd{will efficiently find a lower bound on the number of failures required to create an }\DIFaddend aggregation-induced black \DIFdelbegin \DIFdel{holes}\DIFdelend \DIFaddbegin \DIFadd{hole}\DIFaddend .
...
for regular expressions such as $r^+$, \DIFaddbegin \DIFadd{which is simply syntactic sugar for $r \cdot r^*$, }\DIFaddend a sequence of one or more occurrences of $r$, when appropriate.
