...
%DIF DEL paper.tex          Tue Jun  7 10:15:16 2016
%DIF ADD paper-edited.tex   Thu Jun  9 17:37:43 2016
...
%
Finally, there is an edge from each node in the PGIR such that $P(v) \neq \emptyset$ to the special end node $e$. \DIFaddbegin \DIFadd{We say that $x \leq_{rank} y$ if either $P(x) = P(y) = \emptyset$ or $min ~ P(x) \leq min ~ P(y)$, which simply states that paths that end at PGIR node $x$ are better than (lower rank) than paths that end at $y$.
}\DIFaddend 
...
%
For example, the path $X \cdot D \cdot C \cdot A \cdot W$ is a valid path in the topology that BGP route announcements might take, which would lead to obtaining \DIFaddbegin \DIFadd{a path with }\DIFaddend the most prefered preference \DIFaddbegin \DIFadd{of }\DIFaddend $1$.
...
\DIFdelbegin \DIFdel{We could solve the first problem by filtering routes that go through $D$ at $B$, however , we can not fix the second problem---}\DIFdelend \DIFaddbegin 

\DIFadd{The soundness violation could be fixed by tagging and filtering route advertisements appropriately to have }\DIFaddend $C$ \DIFdelbegin \DIFdel{has made the wrong choice}\DIFdelend \DIFaddbegin \DIFadd{reject routes that went through $E$, however the completeness violation can not be fixed}\DIFaddend . In fact, this policy can not be implemented in BGP in a way that is policy compliant under all failures. \DIFaddbegin \DIFadd{The problem is that $D$ might "regret" its choice depending on what failures have occurred downstream.  
}\DIFaddend 
...
\DIFdelbegin \DIFdel{The more challenging case is when a topology location occurs in mutliple contexts in the PGIR. }\DIFdelend \DIFaddbegin \DIFadd{The problem of deciding local preferences for each router is closely reflected in the structure of the PGIR. In particular, whenever a given router appears in multiple different contexts in the PGIR (i.e., as multiple PGIR nodes), then the compiler must decide which context to prefer.
%DIF > 
}\DIFaddend For example, in \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend compilation example from Figure~\ref{fig:example-compilation}, the topology node $C$ can receive an \DIFdelbegin \DIFdel{announcement }\DIFdelend \DIFaddbegin \DIFadd{advertisement }\DIFaddend from $E$ \DIFdelbegin \DIFdel{and later achieve the backup path, }\DIFdelend \DIFaddbegin \DIFadd{in context $(C,-,2)$ }\DIFaddend or it can receive an \DIFdelbegin \DIFdel{announcement }\DIFdelend \DIFaddbegin \DIFadd{advertisement }\DIFaddend from its neighbor $D$ \DIFdelbegin \DIFdel{and later achieve either the primary or backup path. 
Is it safe for $C$ to prefer its neighbor }\DIFdelend \DIFaddbegin \DIFadd{in context $(C,3,2)$. 
%DIF > 
The compiler's task is to determine a }\emph{\DIFadd{total ordering}} \DIFadd{of contexts for each router in the topology, which reflects the relative preference of advertisements received in each context. For example, if the compiler determines that context $(C,3,2)$ is preferred to context $(C,-,2)$, written as $(C,3,2) < (C,-,2)$, then $C$ will prefer advertisements heard from }\DIFaddend $D$ over \DIFdelbegin \DIFdel{its other neighbor }\DIFdelend $E$\DIFdelbegin \DIFdel{? The important observation is that , if $C$ prefers $D$, then it is never worse off---it will always achieve }\DIFdelend \DIFaddbegin \DIFadd{.
}

\para{Regret-free Preferences}

\DIFadd{How should the compiler determine a total ordering of PGIR contexts for each router in a way that is }\emph{\DIFadd{sound}} \DIFadd{and }\emph{\DIFadd{complete}} \DIFadd{under all possible failures? To this end, we introduce }\emph{\DIFadd{regret-free}} \DIFadd{preferences, a sufficient condition for correctness motivated by the observations from our previous example. Intuitively, preferences are }\emph{\DIFadd{regret-free}} \DIFadd{if, whevever some router $r$ prefers an advertisement $X$ over another $Y$, there is always some policy-compliant path to each final (advertisement) destination that is }\DIFaddend at least as good \DIFdelbegin \DIFdel{a path as }\DIFdelend \DIFaddbegin \DIFadd{as any path }\DIFaddend if it had \DIFdelbegin \DIFdel{choosen $E$.
...
}\DIFdelend \DIFaddbegin \DIFadd{accepted $Y$ instead.
}\DIFaddend 
...
\DIFdel{relation. For example, node $(C,3,2)$ can be preferred to node $(C,-,2)$, but not the other way around. Intuitively, a node }\DIFdelend \DIFaddbegin \DIFadd{We now explain this property more formally in terms of the structure of the PGIR. We use the notation $path(s,d)$ to say that there exists a path in the PGIR from $s$ to $d$. 
%DIF > 
Preferences (for the PGIR) are }\emph{\DIFadd{regret-free}} \DIFadd{if, given (i) any combination of failed topology links}\footnote{\DIFadd{Note that a single failure in the topology may correspond to zero or more failed nodes/edges in the PGIR}}\DIFadd{, (ii) a fixed topology (advertisement) destination $d$, and (iii) two contexts $N_1 < N_2$, then whever there exists a $d'$ such that $loc(d') = d$ and $path(N_2,d')$, then there exists a $d''$ such that $loc(d'') = d$ and $path(N_1,d'')$ with $d'' \leq_{rank} d'$.
%DIF > 
The property simply says that if router $N$ prefers context }\DIFaddend $N_1$ \DIFdelbegin \DIFdel{can be preferred to another node }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend $N_2$\DIFdelbegin \DIFdel{when, for each preference }\DIFdelend \DIFaddbegin \DIFadd{, then any time there is a path from }\DIFaddend $N_2$ \DIFdelbegin \DIFdel{might achieve, there is a better preference that }\DIFdelend \DIFaddbegin \DIFadd{to some destination $d$, then there exists a better (lower ranked) path from }\DIFaddend $N_1$ \DIFdelbegin \DIFdel{will achieve regardless of failures. Formally this means that $N_1$ can be preferred to $N_2$ when:
}\DIFdelend \DIFaddbegin \DIFadd{to destination $d$. 
}\DIFaddend %
\DIFdelbegin \begin{displaymath}\DIFdel{\forall i, \exists j, j \leq i \wedge protect(G_j, N_1, G_i, N_2)}\end{displaymath}
%DIFAUXCMD
%DIF < 
\DIFdel{where }\textit{\DIFdel{protect}} %DIFAUXCMD
\DIFdel{means that, from $N_1$ on the product graph restricted to nodes that can potentially achieve preference $j$ or better, }\DIFdelend \DIFaddbegin \DIFadd{For example, the compilation example in Figure~\ref{fig:example-compilation}, a choice for $C$ to prefer context $(C,3,2)$ to $(C,-,2)$ would be regret-free, since }\DIFaddend there is always \DIFaddbegin \DIFadd{at least as good }\DIFaddend a path to \DIFdelbegin \DIFdel{each ending location whenever this is a path from $N_2$ restricted to $G_i$. }\DIFdelend \DIFaddbegin \DIFadd{destination $W$ regardless of failures. 
}\DIFaddend 



\DIFaddbegin \para{A Preference Inference Algorithm}
...
\DIFadd{Clearly searching for }\emph{\DIFadd{regret-free}} \DIFadd{preferences by enumerating all possible combinations of failures and preference orderings is intractable. We thus adopt a conservative analysis that we found to be highly-effective in practice. The idea is to search for regret-free preferences while also refining the search when possible by taking into account information about where advertisements must have already traversed. 
}
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \Procedure{Protect($G$, $N_1$, $N_2$)}{}
...
\DIFaddbegin \DIFadd{The analysis determines that context $N_1 < N_2$ if $N_1$ }\textit{\DIFadd{protects}} \DIFadd{against the failures of $N_2$.
%DIF > 
}\DIFaddend Algorithm~\ref{alg:failures} defines what it means for one node to \textit{protect} against the failures of another. \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{The algorithm }\DIFaddend walks from nodes $N_1$ and $N_2$ \DIFdelbegin \DIFdel{in $G_j$ and $G_i$ respectively, and }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend ensures that for every \textit{step} $N_2$ can take to some new topology location, $N_1$ can, at the very least, take an equivalent step. 
%DIF > 
When there is no equivalent step, the algorithm \DIFaddbegin \DIFadd{attempts to take into account where the advertisement must have already traversed to have arrived at $N_1$. In particular, it }\DIFaddend checks to see if there is an equivalent dominator (a topology location the advertisement must have passed through earlier) that can protect against failures instead.
\DIFdelbegin \DIFdel{Thus any time }\DIFdelend \DIFaddbegin 

\DIFadd{By repeating this process, we are ensured that any time $N$ receives an advertisement in both context }\DIFaddend $N_2$ \DIFdelbegin \DIFdel{can get to a final location for preference $i$}\DIFdelend \DIFaddbegin \DIFadd{and $N_1$}\DIFaddend , \DIFaddbegin \DIFadd{$N$ can safely prefer }\DIFaddend $N_1$\DIFdelbegin \DIFdel{can get to at least the same final locations for a better preference $j$. The algorithm }\DIFdelend \DIFaddbegin \DIFadd{, knowing that BGP will obtain end-to-end paths which are at least as good as it would have if $N$ had preferred $N_2$. Algorithm~\ref{alg:failures} }\DIFaddend terminates since the number of related states $(x,y)$ that can be \DIFdelbegin \DIFdel{expolored }\DIFdelend \DIFaddbegin \DIFadd{explored }\DIFaddend is finite.
...
\DIFdelend \DIFaddbegin \DIFadd{$(<)$ }\DIFaddend relation. If two nodes are incomparable, \DIFaddbegin \DIFadd{then }\DIFaddend the compiler rejects the policy as unimplementable.

\para{Avoiding Loops}

The checks for failure safety described above overlook one critical point: \DIFdelbegin \DIFdel{Paths for a higher preference might not exist }\DIFdelend \DIFaddbegin \DIFadd{A better (lower rank) path might not be available after some router $N$ }\DIFaddend due to loops. To avoid this situation, the compiler checks that, \DIFdelbegin \DIFdel{when node $x$ protects against failures for node $y$, its does so without using any nodes that shadow another node that }\DIFdelend \DIFaddbegin \DIFadd{any time some context $N_1$ }\DIFaddend appears ``above'' \DIFdelbegin \DIFdel{$x$ }\DIFdelend \DIFaddbegin \DIFadd{(i.e., can reach) $N_2$ }\DIFaddend in the PGIR\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{, then $N_1$ must be strictly preferred to $N_2$. The idea is that anytime $N_2$ is not available to use because the advertisement already went through $N_1$, it ultimately does not matter since $N_1$ will result in strictly better end-to-end paths.
}
...
%
Figure~\ref{fig:compilation-times} shows the compilation times for data centers (a), and backbone networks (b) of different sizes. For both policies, we measure the mean compilation time per prefix \DIFaddbegin \DIFadd{predicate }\DIFaddend since the compiler operates on each prefix \DIFaddbegin \DIFadd{predicate }\DIFaddend in parallel. \DIFaddbegin \DIFadd{Note that a single predicate can describe many prefixes, for example by matching on a disjunction of prefixes. }\DIFaddend At their largest sizes, the \DIFdelbegin \DIFdel{per-prefix }\DIFdelend \DIFaddbegin \DIFadd{per-predicate }\DIFaddend compilation time is roughly 10 seconds for the data center network and 45 seconds for the backbone network.
