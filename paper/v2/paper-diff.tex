...
%DIF ADD paper-edited.tex   Thu Jun  9 16:23:28 2016
...
\DIFdelend \DIFaddbegin \Procedure{Protect($G$, $N_1$, $N_2$)}{}
  ...
  \While {$!q.Empty()$}
    ...
    \For {$x$ in adj(\DIFdelbegin \DIFdel{$G_2$}\DIFdelend \DIFaddbegin \DIFadd{$G$}\DIFaddend , $n_2$)}
      \If {\big($\exists y \in$ adj(\DIFdelbegin \DIFdel{$G_1$}\DIFdelend \DIFaddbegin \DIFadd{$G$}\DIFaddend ,$n_1$), shadows $x$\big) \textbf{or} \\  
        ...
        \If {$(x,y)$ not marked \DIFaddbegin \DIFadd{$\wedge$ $x \leq_{rank} y$}\DIFaddend }
