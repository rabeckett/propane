...
%DIF ADD paper-edited.tex   Fri Jun 10 10:34:33 2016
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \subsection{\DIFadd{More Safety}}
...
}\CD{PG1} \DIFadd{\Path \text{ }}\End\DIFadd{(}\CD{A}\DIFadd{), where }\CD{PG1} \DIFadd{falls under the }\CD{PG} \DIFadd{aggregate. Figure~\ref{fig:aggregation-safety} shows the network topology simplified PGIR. Since the compiler knows aggregation will occur at $X$, and it knows that the }\CD{PG1} \DIFadd{prefix will originate at $A$, we can compute the number of failures it would take to disconnect $A$ from $X$. We could remove the $A$--$D$--$X$ path first. We would then need to remove any other $A$--$D$ or $D$--$X$ links from the PGIR (in this case none). Next, we could remove the links along the $A$--$C$--$X$ path, repeating the process. Because $A$ is then disconnected from $X$, the compiler knowns that 2 is a lower bound on the number of failures for aggregation safety for prefix }\CD{PG1}\DIFadd{. This process is repeated for other aggregation locations (e.g., $Y$).
}
