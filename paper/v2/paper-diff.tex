...
%DIF ADD paper-edited.tex   Thu Jun  9 11:18:36 2016
...
%DIF > 
}\DIFaddend For example, in \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend compilation example from Figure~\ref{fig:example-compilation}, the topology node $C$ can receive an \DIFdelbegin \DIFdel{announcement }\DIFdelend \DIFaddbegin \DIFadd{advertisement }\DIFaddend from $E$ \DIFdelbegin \DIFdel{and later achieve the backup path, }\DIFdelend \DIFaddbegin \DIFadd{in one context }\DIFaddend or it can receive an \DIFdelbegin \DIFdel{announcement }\DIFdelend \DIFaddbegin \DIFadd{advertisement }\DIFaddend from its neighbor $D$ \DIFdelbegin \DIFdel{and later achieve either the primary or backup path. Is it safe for $C$ to prefer its neighbor $D$ over its other neighbor $E$? The important observation is that, if $C$ prefers $D$, then it is never worse off---it will always achieve at least as good a path as if it had choosen $E$}\DIFdelend \DIFaddbegin \DIFadd{in a different context. The compiler's task is to determine a }\emph{\DIFadd{total ordering}} \DIFadd{of contexts for each router in the topology, which reflects the relative preference of advertisements received in that context}\DIFaddend . For example, \DIFdelbegin \DIFdel{suppose $C$ chooses a route from $D$, but cannot achieve its primary path because the $A$ -- $W$ link has failed. In this case, the advertisement from $C$ will still be sent along towards the ultimate backup location $W$. Since the }\DIFdelend \DIFaddbegin \DIFadd{if the compiler determines that context }\DIFaddend $(C,3,2)$ \DIFdelbegin \DIFdel{node has the same one-step and two-step next hops as node }\DIFdelend \DIFaddbegin \DIFadd{is preferred $(<)$ to context }\DIFaddend $(C,-,2)$\DIFdelbegin \DIFdel{in the product graph, no possible failure will prevent a route advertisment from reaching $(W,-,4)$ that wouldn't have otherwise prevented it if $C$ had choosen }\DIFdelend \DIFaddbegin \DIFadd{, then $C$ will need to prefer advertisements heard from }\DIFaddend $E$ \DIFaddbegin \DIFadd{over $D$}\DIFaddend .

\DIFaddbegin \para{Regret Freedom}
...
\DIFaddend We apply \sysname on real policies for backbone and data center networks. Our mains goals are to evaluate if its front-end is expressive enough for real-world policies and the time the compiler takes to generate router configurations.
...
%DIFDELCMD < %%%
\DIFdelend \subsection{Expressiveness}
