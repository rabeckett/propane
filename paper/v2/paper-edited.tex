...
%
The preference function $P$ for node $n = (l, a_1, \dots, a_k)$ is defined as $P(n) = \set{i~\vert~a_i \in F_i} $. That is, it records which preferences are achieved from each policy automaton for every node in the product construction.
%
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=.6\columnwidth]{figures/loops}
  \caption{A Product Graph where preference inference is unsound before considering loops.}
