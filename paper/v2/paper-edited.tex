...
%
Finally, there is an edge from each node in the PGIR such that $P(v) \neq \emptyset$ to the special end node $e$. We use the notation $x \leq_{rank} y$ as a shorthand for $min ~ P(x) \leq min ~ P(y)$, which simply states that paths that end at PGIR node $x$ are better than (lower rank) than paths that end at $y$.
  ...
  \While {$!q.Empty()$}
    ...
    \For {$x$ in adj($G$, $n_2$)}
      \If {\big($\exists y \in$ adj($G$,$n_1$), shadows $x$\big) \textbf{or} \\  
        ...
        \If {$(x,y)$ not marked $\wedge$ $x \leq_{rank} y$}
