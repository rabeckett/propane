...
Once \sysname compiles the policy, it is guaranteed to be compliant under all possible failure scenarios, modulo aggregation black holes (which are unavoidable in general). In the presence of aggregation, the \sysname compiler will also efficiently find a lower bound on the number of failures required to create an aggregation-induced black hole.
