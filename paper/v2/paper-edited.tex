...
\CD{R2}-\CD{Prov} link but to announce routes with communities for the \CD{R2}-\CD{Cust} and \CD{R1}-\CD{Cust} links. Finally, to implement P4 and P5, the operators will have to compute and configure appropriate prefix- and AS-path-based import and export filters at each router.
...
\end{code}
\noindent
The \CD{NoTrans} constraint above requires that all traffic not follow a path that transits the network between \CD{Peer} and \CD{Prov}.
...
%
Finally, there is an edge from each node in the PGIR such that $P(v) \neq \emptyset$ to the special end node $e$. We say that $x \leq_{rank} y$ if either $P(x) = P(y) = \emptyset$ or $min ~ P(x) \leq min ~ P(y)$, which simply states that paths that end at PGIR node $x$ are better than (lower rank) than paths that end at $y$.
...
%
For example, the path $X \cdot D \cdot C \cdot A \cdot W$ is a valid path in the topology that BGP route announcements might take, which would lead to obtaining a path with the most prefered preference of $1$.
...
The soundness violation could be fixed by tagging and filtering route advertisements appropriately to have $C$ reject routes that went through $E$, however the completeness violation can not be fixed. In fact, this policy can not be implemented in BGP in a way that is policy compliant under all failures. The problem is that $D$ might "regret" its choice depending on what failures have occurred downstream.  
...
The problem of deciding local preferences for each router is closely reflected in the structure of the PGIR. In particular, whenever a given router appears in multiple different contexts in the PGIR (i.e., as multiple PGIR nodes), then the compiler must decide which context to prefer.
%
For example, in our compilation example from Figure~\ref{fig:example-compilation}, the topology node $C$ can receive an advertisement from $E$ in context $(C,-,2)$ or it can receive an advertisement from its neighbor $D$ in context $(C,3,2)$. 
%
The compiler's task is to determine a \emph{total ordering} of contexts for each router in the topology, which reflects the relative preference of advertisements received in each context. For example, if the compiler determines that context $(C,3,2)$ is preferred to context $(C,-,2)$, written as $(C,3,2) < (C,-,2)$, then $C$ will prefer advertisements heard from $D$ over $E$.

\para{Regret-free Preferences}

How should the compiler determine a total ordering of PGIR contexts for each router in a way that is \emph{sound} and \emph{complete} under all possible failures? To this end, we introduce \emph{regret-free} preferences, a sufficient condition for correctness motivated by the observations from our previous example. Intuitively, preferences are \emph{regret-free} if, whevever some router $r$ prefers an advertisement $X$ over another $Y$, there is always some policy-compliant path to each final (advertisement) destination that is at least as good as any path if it had accepted $Y$ instead.
...
%
For example, the compilation example in Figure~\ref{fig:example-compilation}, a choice for $C$ to prefer context $(C,3,2)$ to $(C,-,2)$ would be regret-free, since there is always at least as good a path to destination $W$ regardless of failures. 
...
Clearly searching for \emph{regret-free} preferences by enumerating all possible combinations of failures and preference orderings is intractable. We thus adopt a conservative analysis that we found to be highly-effective in practice. The idea is to search for regret-free preferences while also refining the search when possible by taking into account information about where advertisements must have already traversed. 

\begin{algorithm}[t!]
\caption{Failure Protection}
\label{alg:failures}
\begin{algorithmic}[1]
\Procedure{Protect($G$, $N_1$, $N_2$)}{}
...
The analysis determines that context $N_1 < N_2$ if $N_1$ \textit{protects} against the failures of $N_2$.
%
Algorithm~\ref{alg:failures} defines what it means for one node to \textit{protect} against the failures of another. The algorithm walks from nodes $N_1$ and $N_2$ and ensures that for every \textit{step} $N_2$ can take to some new topology location, $N_1$ can, at the very least, take an equivalent step. 
%
When there is no equivalent step, the algorithm attempts to take into account where the advertisement must have already traversed to have arrived at $N_1$. In particular, it checks to see if there is an equivalent dominator (a topology location the advertisement must have passed through earlier) that can protect against failures instead.

By repeating this process, we are ensured that any time $N$ receives an advertisement in both context $N_2$ and $N_1$, $N$ can safely prefer $N_1$, knowing that BGP will obtain end-to-end paths which are at least as good as it would have if $N$ had preferred $N_2$. Algorithm~\ref{alg:failures} terminates since the number of related states $(x,y)$ that can be explored is finite.

For each router in the topology, local preferences are now obtained by sorting the corresponding PGIR contexts according to the $(<)$ relation. If two nodes are incomparable, then the compiler rejects the policy as unimplementable.

\para{Avoiding Loops}

The checks for failure safety described above overlook one critical point: A better (lower rank) path might not be available after some router $N$ due to loops. To avoid this situation, the compiler checks that, any time some context $N_1$ appears ``above'' (i.e., can reach) $N_2$ in the PGIR, then $N_1$ must be strictly preferred to $N_2$. The idea is that anytime $N_2$ is not available to use because the advertisement already went through $N_1$, it ultimately does not matter since $N_1$ will result in strictly better end-to-end paths.
