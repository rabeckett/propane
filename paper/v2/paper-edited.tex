...
high-level constraints on both the shape and relative preferences of traffic paths.
%
...
\begin{figure*}[t]\small
  ...
  \[ \begin{array}{rclr}
    \hline
     ...
     cc     &::=& agg(x, lp) \BNFALT tag(c, t, lp) & \textit{control constraints} \\
  \end{array} \]
...
Throughout the remainder of the paper, we will the convention that metavariables $m$ and $n$ stand for PGIR nodes and $l$ stands for a topology location. Captial letters like $X$, $Y$ refer to concrete topology locations, while capital letters with subscripts such as $X_1$ and $X_2$ refer to concrete PGIR nodes that share a topology location (\IE, $\tilde{X_1}$ = $\tilde{X_2} = X$).
...
\begin{figure}[t!]
  \centering
  \includegraphics[width=.66\columnwidth]{figures/loops}
