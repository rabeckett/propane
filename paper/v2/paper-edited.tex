...
leave through \CD{Peer} and so then \CD{Prov}.  As we
...
Line 3 applies to any other traffic, and allows the traffic to leave through any direct neighbor with the usual preferences of \CD{R1} over \CD{R2} over \CD{Peer} over \CD{Prov}. To summarize our progress, the \CD{Routing} policy
...
\end{code}
\noindent
Here we define transit traffic between $X$ and $Y$ as traffic that enters the network through $X$ or $Y$ and then leaves the network through $X$ or $Y$. Similarly, we define customer transit for customer $X$ and provider $Y$ as traffic that later goes through both $X$ and $Y$ after leaving our network. Using these two new abstractions, we can now implement policies P2 and P5 with the following constraint.
...
\end{code}
\noindent
The \CD{NoTrans} constraint above requires that all traffic not follow a path that transits the network between \CD{Peer} and \CD{Prov}. Additionally, it prevents traffic from ever following paths that leave our network and later pass through both \CD{Prov} and \CD{Cust}.  To implement both \CD{Routing}
...
The special keyword \Out{} represents any location outside the user's network, and the keyword \In{} represents any location inside the network.
...
As before, once \sysname compiles the policy, it is guaranteed to be policy-compliant under all possible failure scenarios. In addition, \sysname will efficiently find a lower bound on the number of failures required to create an aggregation-induced black hole.
...
for regular expressions such as $r^+$, which is simply syntactic sugar for $r \cdot r^*$, a sequence of one or more occurrences of $r$, when appropriate.
