...
%
We say that a PGIR node $x_1$ is a \emph{shadow} of topology location $x$ when $x_1 \in V'$ and $loc(x_1) = x$. As a shorthand, we write $x_1 \approx x_2$ when $x_1$ and $x_2$ are both shadows for the same topology location.
%
...
%
The soundness violation could be fixed by tagging and filtering route advertisements appropriately so that $C$ rejects routes that go through $E$, however the completeness violation cannot be fixed. In fact, this policy cannot be implemented in BGP in a way that is policy compliant under all failures.


\para{Problem Formulation}

The problem of determining local preferences for each router is closely reflected in the structure of the PGIR. In particular, whenever a given router appears in multiple different contexts in the PGIR (i.e., as multiple PGIR nodes that are shadows of the router), then the compiler must decide which shadow to prefer.
%
