...
%
%Throughout the remainder of the paper, we will use metavariables without subscripts such as $x$ to represent topology locations, and we will use similar metavariables with subscripts such as $x_1$ or $x_2$ to represent shadows of $x$ (i.e., PGIR nodes for location $x$).
...
The problem of determining local preferences for each router is closely reflected in the structure of the PGIR. In particular, whenever a given router appears in multiple shadow nodes in the PGIR, then the compiler must decide which shadow to prefer.
%
In our compilation example from Figure~\ref{fig:example-compilation}, the topology node $C$ can receive an advertisement from $E$ in shadow $(C,-,2)$ or it can receive an advertisement from its neighbor $D$ in shadow $(C,3,2)$.
%
The compiler's task is to determine a \emph{total ordering} of shadow nodes for each router in the topology, which reflects the relative preference of advertisements received in each shadow. For example, if the compiler determines that, for router $C$, shadow $(C,3,2)$ can be locally preferred to shadow $(C,-,2)$, written as $(C,3,2) \leq_{lp} (C,-,2)$, then this means that $C$ will prefer advertisements heard from $D$ (with tag $(2,2)$) over $E$ (with tag $(-,2)$). Section~\ref{sec:abgp} discusses our encoding of preferences and filters in more detail.

\para{Regret-free Preferences}

A natural question then is, how should the compiler order PGIR shadows ($\leq_{lp}$) for each router in a way that is both \emph{sound} and \emph{complete} under all possible failures? To this end, we introduce the notion of \emph{regret-free} preferences, motivated by the observations from the example in Figure~\ref{fig:unimplementable}. We say a router $r$ has a \emph{regret-free} preference for a set of advertisements $X$ over $Y$ if, whevever $r$ selects an advertisement from $X$ over another from $Y$, there is always some policy-compliant path to each final (advertisement) destination $d$ that is at least as good $(\leq_{rank})$ as any possible path to $d$ if it had accepted an advertisement from $Y$ instead. In other words, the preference of $X$ over $Y$ at $r$ is regret-free if we are never worse off by choosing an advertisement from $X$ when available. The notion of regret-free preferences can be lifted to nodes in the PGIR by considering the set of advertisments available to each node.

In the compilation example from Figure~\ref{fig:example-compilation}, the choice for $C$ to prefer shadow $(C,3,2)$ to $(C,-,2)$ is regret-free, since there will always be at least as good a path to destination $W$ regardless of any failures that might occur in the network. For example, if the $C$--$A$ link fails, then there is still a backup path from $(C,3,2)$ to $(W,-,4)$ that is just as good as any path from $(C,-,2)$. Likewise, any combination of failures to disconnect $(C,3,2)$ from $(W,-,4)$ would also disconnect $(C,-,2)$ from $(W,-,4)$.
...
Searching precisely for \emph{regret-free} preferences in general is hard, and clearly enumerating all possible combinations of failures and preference orderings is intractable. We thus adopt a conservative analysis that we found to be both effective and efficient in practice. The idea is to $i)$ search for regret-free preferences by comparing the set of paths available after accepting advertisments in two different PGIR shadows $N_1$ and $N_2$ of topology location $N$, and $ii)$ refine the comparison when necessary by considering where the announcements must have traversed before arriving at $N_1$ or $N_2$.
...
\Procedure{Regret-free($G$, $N_1$, $N_2$)}{}
  ...
  \While {$!q.Empty()$}
    ...
    \If {$n_1 \nleq_{rank} n_2$}
      \Return false
    \EndIf
    \For {$n_2'$ in adj($G$, $n_2$)}
      \If {\big($\exists n_1' \in \textup{adj}(G,n_1), n_1' \approx n_2'$\big) \textbf{or} \\
            \hspace{5.2em} 
            \big($\exists n_1' \in$ $G$, dominates $n_1$, $n_1' \approx n_1'$\big)}
        \If {$(x,y)$ not marked}
...
For each router in the topology, local preferences are now obtained by sorting the corresponding PGIR contexts according to the $(\leq_{lp})$ relation determined by the \textup{Regret-Free} algorithm. If two nodes are incomparable, then the compiler rejects the policy as unimplementable.
...
To avoid this situation, the compiler checks that, any time we have two shadows $N_1$ and $N_2$ for some topology location $N$, where $N_1$ appears ``above'' (i.e., can reach) $N_2$ in the PGIR, then $N_1$ must be strictly preferred to $N_2$ (that is, $N_1 <_{lp} N_2$). The idea is that any time $N_2$ is not usable because the advertisement already went through $N_1$, it ultimately does not matter since $N$ has a regret-free preference for $N_1$ anyway.
%
...
As shown in \S\ref{sec:motivation}, BGP aggregation can lead to subtle black-holing of traffic when failures occur. Determining when this can happen requires knowledge, not only of the topology, but also of the routing policy. For instance, a policy might require all traffic for a particular prefix to go over a single link before being aggregated, even when there are several available in the toplogy. If that one link fails, a black hole might be introduced.
%
...
We frame the aggregation problem as a variant of the min-cut problem in the PGIR. Specifically, for each prefix that falls under an aggregate, we are interested in finding a lower bound on the number of failures required to disconnect the prefix's origin from its aggregation point. The difficulty, however, is that links in the topology can appear in multiple places in the PGIR, thus preventing the direct application of a standard min-cut algorithm.
%
...
the National Science Foundation award CNS-1161595.
