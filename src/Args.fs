...
let setFailures s = 
  ...
  | _ -> 
    ...
    if i < 0 then raise (InvalidArgException("Invalid number: " + s))
    failures := Concrete i
...
let setDebugDir s = 
  if Path.IsPathRooted(s) then debugDir := s + sep
...
let args = 
  [| ("-pol", String(fun s -> polFile := Some(setFile s)), "Policy file")
     ...
     ("-stats:on|off", String(setOnOff stats "stats"), 
      "Display performance statistics to stdout (default off)")
     
     ("-minimize:on|off", String(setOnOff minimize "minimize"), 
     ...
     ("-prepending:on|off", String(setOnOff usePrepending "prepending"), 
      "Use AS path prepending (default off)")
     
     ("-no-export:on|off", String(setOnOff useNoExport "no-export"), 
      "Use no-export community (default off)")
     ("-ibgp:on|off", String(setOnOff useIBGP "ibgp"), "Assume IBGP configuration (default off)")
     
     ("-checkenter:on|off", String(setOnOff checkEnter "check enter"), 
      "Check traffic entry conditions to network")
     
     ("-debug:on|off", String(setOnOff debug "debug"), 
      "Log/Save Debugging information (default off)")
...
let parse (argv : string []) : unit = 
  if Array.isEmpty argv then exit()
  else 
    try 
    ...
    if !debug then cleanDir !debugDir
    settings := Some { PolFile = !polFile
                       TopoFile = !topoFile
                       OutFile = !outFile
                       Anycast = !anycast
                       UseMed = !useMed
