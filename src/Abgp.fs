...
module PrefixWide = 
    ...
    let removeUnreachableRules (cg: CGraph.T) (pb: Route.PredicateBuilder) (config: Map<string,Actions>) = 
        let inline anyValue f vs =
            if Set.isEmpty vs 
            then pb.True 
            else Set.fold (fun x y -> pb.Or(x, f y)) pb.False vs
        ...
        let aux router actions = 
            ...
            let anyPeer = anyValue pb.Location peers
            ...
            | Filters fs -> 
                ...
                let acc = ref (pb.Not( pb.And(anyComm,anyPeer) ))
                List.choose (fun f ->
                    let oldAcc = !acc
                    ...
                    | Allow((m,_),_) -> 
                        let p = 
                            match m with 
                            | Peer p -> predOfPeer p
                            | State(c,p) -> pb.And(pb.Community c, predOfPeer p)
                            | PathRE _ -> isRE <- true; pb.False
                        acc := pb.Or(!acc, p) 
                    if !acc <> oldAcc || isRE then Some f else None) fs 
                |> Filters
...
module RouterWide = 
    ...
    let coveringMatch m1 m2 = 
        match m1, m2 with
        | Peer x, Peer y -> if coveringPeer x y then Yes else No
        | Peer x, State (d,y) -> if coveringPeer x y then ForComm d else No
        | State(_,x), Peer y -> if coveringPeer x y then Yes else No
    ...
    let rec catchAll (pb: Route.PredicateBuilder) allComms covered (p1,f1) below : bool = 
        ...
        | [] -> false
        | ((p2,f2) as pair)::tl ->
            if disjointFilter f1 f2 && pb.And(p1,p2) = pb.False then 
                catchAll pb allComms covered (p1,f1) tl
            else
    ...
    let getAllCommunities (config : T) = 
        ...
        Update.updateMod (fun _ _ _ _ m ->
            match m with 
            | SetComm c -> comms := Set.add c !comms; Some m
...
let genConfig (cg: CGraph.T) 
    ...
    config <- 
        if settings.Minimize
        then PrefixWide.minimize cg pb config
        else config
...
let inline insideLoc v = 
    if Topology.isInside v.Node 
    then Some (CGraph.loc v) 
    else None


let inline insideOriginatorLoc v =
    if (Topology.isInside v) && (Topology.canOriginateTraffic v) 
    then Some v.Loc 
    else None
...
let compileToIR fullName idx pred (polInfo: Ast.PolInfo option) aggInfo (reb: Regex.REBuilder) res : PrefixCompileResult =
    ...
    try
        ...
        match ordering with
        | Ok ord ->
            let config, configTime = Profile.time (genConfig cg pb pred ord) inExports
            let result = 
                {K=k; 
                 BuildTime=buildTime; 
...
let toConfig (abgp: T) = 
    ...
    for kv in abgp.RConfigs do 
        ...
        if not inPeers.IsEmpty then
            let internalStr = matchAllPeers inPeers
            createAsPathList(Config.Kind.Permit, asPathMap, asPathLists, List(), alID, internalStr) |> ignore
        if not outPeers.IsEmpty then
            let externalStr = matchAllPeers outPeers
            createAsPathList(Config.Kind.Permit, asPathMap, asPathLists, List(), alID, externalStr) |> ignore
            ...
            for Route.TrafficClassifier(prefix, comms, negComms) in tcs do 
                ...
                | Filters fs ->
                    ...
                    for f in fs do 
                        ...
                        // function to translate an ABGP peer to an appropriate as-path filter
                        let ppol = peerPol asPathMap asPathLists als alID
