...
let inline validIndex i = (i >= 0 && i < 32)
let empty : T = 0<B>
...
let inline union (x : T) (y : T) : T = Int32WithMeasure<B>(int x ||| int y)
let inline intersect (x : T) (y : T) : T = Int32WithMeasure<B>(int x &&& int y)
let inline negate (x : T) : T = Int32WithMeasure<B>(~~~(int x))
let inline difference (x : T) (y : T) : T = intersect x (negate y)
let inline isEmpty (x : T) : bool = (int x = 0)
...
let inline filter f (x : T) = 
  ...
  let mutable acc = x
  for i = 0 to 31 do
    if (v &&& 1) = 1 && not (f i) then acc <- remove i acc
    v <- (v >>> 1)
  acc
...
let inline toSet (x : T) = 
  let rec aux i v set = 
    if i >= 32 then set
    elif (v &&& 1) = 1 then aux (i + 1) (v >>> 1) (Set.add i set)
