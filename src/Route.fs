...
type Node = 
   | Leaf of Ranges
   | Node of Var * HashCons<Node> * HashCons<Node>
   ...
   override x.GetHashCode() = 
      match x with
      | Leaf r -> hash r
      | Node(v, l, r) -> 19 * (19 * l.Hash + r.Hash) + hash v
...
type BddBuilder(order : Var -> Var -> int) = 
   ...
   let mkNode (node : Node) : HashCons<Node> = builder.Hashcons node
   let leaf v = mkNode (Leaf v)
   
   let node (v, l : HashCons<Node>, r : HashCons<Node>) = 
      if l = r then l
   ...
   let rec negate (n : HashCons<Node>) = 
      match n.Node with
      | Leaf v -> leaf (negateRanges v)
      | Node(v, l, r) -> node (v, negate l, negate r)
   
   let rec apply f (n1 : HashCons<Node>) (n2 : HashCons<Node>) = 
   ...
   member __.NodeCount() = builder.Count()
   ...
   member __.ToString(n) = 
      let rec fmt depth (n : HashCons<Node>) = 
         match n.Node with
         | Leaf v -> 
   ...
   struct
      ...
      override v.ToString() = 
         if v.IsTemplate then sprintf "%s" v.Name
            ...
            else if r.Lo > v.Slash then 
               sprintf "%d.%d.%d.%d/%d le %d ge %d" a b c d v.Slash r.Lo r.Hi
